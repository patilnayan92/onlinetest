...
import os
from nayan.settings import SERVER_POOL_PORT

SCRIPT_DIR = os.path.abspath(os.path.dirname(__file__))
TARGET_CONTAINER_NAME = 'nayan_code_server'
SRC_IMAGE_NAME = 'fossee/nayan_codeserver'
CHECK_FILE = 'server_running.txt'
CHECK_FILE_PATH = os.path.join(SCRIPT_DIR, 'nayan_data', CHECK_FILE)
...
def start(ctx, ports=SERVER_POOL_PORT, image=SRC_IMAGE_NAME, unsafe=False,
    ...
    if unsafe:
        with ctx.cd(SCRIPT_DIR):
            ...
            ctx.run("sudo python{0} -m nayan.code_server".format(
                    version
                )
            )
    else:
        cmd_params = {'ports': ports,
            ...
            'vol_mount': os.path.join(SCRIPT_DIR, 'nayan_data'),
            'command': 'sh {0}'.format(
                os.path.join(SCRIPT_DIR,
                'nayan_data', 'nayan', 'scripts', 'nayan_script.sh')
            )
        }
        ...
        create_dir(os.path.join(SCRIPT_DIR, 'nayan_data', 'data'))
        create_dir(os.path.join(SCRIPT_DIR, 'nayan_data', 'output'))

        ctx.run('cp -r {0} {1}'.format(
                os.path.join(SCRIPT_DIR, 'nayan'),
                os.path.join(SCRIPT_DIR, 'nayan_data')
            )
        )

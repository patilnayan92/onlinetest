...
from nayan.code_server import(submit,
...
FIXTURES_DIR_PATH = os.path.join(settings.BASE_DIR, 'nayan', 'fixtures')
...
def get_model_class(model):
    ctype = ContentType.objects.get(app_label="nayan", model=model)
...
class Course(models.Model):
    ...
    def create_demo(self, user):
        course = Course.objects.filter(creator=user, name="nayan Demo course")
        if not course:
            course = Course.objects.create(name="nayan Demo course",
                                           enrollment="open",
                                           creator=user)
            quiz = Quiz()
...
class Question(models.Model):
    ...
    def get_test_cases(self, **kwargs):
        ...
        for tc in self.testcase_set.values_list("type", flat=True).distinct():
            test_case_ctype = ContentType.objects.get(app_label="nayan",
                                                      model=tc)
...
class Quiz(models.Model):
    ...
    def create_demo_quiz(self, course):
        demo_quiz = Quiz.objects.create(
            ...
            attempts_allowed=-1,
            time_between_attempts=0,
            description='nayan Demo quiz', pass_criteria=0,

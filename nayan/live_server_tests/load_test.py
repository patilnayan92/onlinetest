...
from nayan.models import User, Profile, Question, Quiz, Course, QuestionPaper, TestCase
from nayan.code_server import ServerPool
from nayan import settings

from .selenium_test import SeleniumTest


class NayanSeleniumTests(StaticLiveServerTestCase):
    @classmethod
    def setUpClass(cls):
        super(NayanSeleniumTests, cls).setUpClass()
        # setup a demo code server
        settings.code_evaluators['python']['standardtestcase'] = \
            "nayan.python_assertion_evaluator.PythonAssertionEvaluator"
        settings.code_evaluators['c']['standardtestcase'] = \
            "nayan.cpp_code_evaluator.CppCodeEvaluator"
        settings.code_evaluators['bash']['standardtestcase'] = \
            "nayan.bash_code_evaluator.BashCodeEvaluator"
    ...
    def tearDownClass(cls):
        ...
        super(NayanSeleniumTests, cls).tearDownClass()

    def test_load(self):
        url = '%s%s' % (self.live_server_url, '/exam/login/')
        quiz_name = "Nayan Demo quiz"

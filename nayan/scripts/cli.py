...
def main():
    ...
    parser = argparse.ArgumentParser(prog="nayan")
    ...
    create_demo_parser.add_argument(
        "path", type=str,
        default=path.join(CUR_DIR, "nayan_demo"),
        nargs="?",
        help="Path to the demo Django project (defaults to 'nayan_demo')"
    )
...
def create_demo(project_name='nayan_demo', project_dir=CUR_DIR):
    ...
    try:
        ...
        print("Error: {0}\nExiting nayan Installer".format(e))
...
def run_server(args=None):
    try:
        from nayan import code_server
        code_server.main(args)
    except Exception as e:
        print("Error: {0}\nExiting nayan code server".format(e))
